...
function App() {
  ...
  const pwdGenerator = useCallback(() => {
    ...
    if (chrAllow) {
      str +=
        "ABC)%*^!<>?DEF@#$GHIJ)%*^!<>?KLMNOP@#$[]QRST)%*^!<>?${}[]YZa)%*^!<>?bcde@#${}[]fgh[]*()%*^!<>?ijklmn[]*()%*^!<>?opqrs[]*()%*^!<>?tuv[]*()%*^!<>?wxyz";
    }
  ...
  const copyToClipboard = useCallback(() => {
    ...
    window.navigator.clipboard
      .writeText(pwd) //there is no window object in serverSide
      .then(() => {
      ...
      <div className="w-full max-w-md mx-auto shadow-md rounded-lg p-6 my-8  text-orange-500 bg-gray-800">
        <div className="flex shadow-rounded-lg  mb-4 font-bold">
          <input
            ...
            value={pwd}
            ref={pwdRef}
            className="outline-none w-full p-2 rounded-md h-10"
            placeholder="password here"
            readOnly
          ></input>
          <button
            onClick={copyToClipboard}
            className="p-2 text-white justify-center bg-blue-700 rounded-md ml-4 hover:bg-blue-800 hover:scale-110"
          >
            Copy
          </button>
        </div>
        ...
        <div className="flex items-center gap-x-1">
          <input
            ...
            className="cursor-pointer"
            onChange={(e2) => {
              setChrAllow((prevState) => !prevState);
            }}
          ></input>
          {/* Here we are setting the prevState varible as checkbox tick in/out */}
